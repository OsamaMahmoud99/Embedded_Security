...
bool ring_buffer_read(ring_buffer_t* rb, uint8_t* byte){
    uint32_t local_read_inxdex = rb->read_index;
    uint32_t local_write_index = rb->write_index;

    if (local_read_inxdex == local_write_index){
        return false;
    }

    *byte = rb->buffer[local_read_inxdex];
    local_read_inxdex = (local_read_inxdex + 1) & rb->mask;
    rb->read_index = local_read_inxdex;
...
bool ring_buffer_write(ring_buffer_t* rb, uint8_t byte){
    ...
    if(next_write_index == local_read_inxdex){
        return false;
    }
