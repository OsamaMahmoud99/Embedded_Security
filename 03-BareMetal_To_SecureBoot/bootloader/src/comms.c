...
uint8_t comms_is_single_byte_packet(const comms_packet_t* packet, uint8_t byte){
    uint8_t flag = 1;
    ...
    while (uart_data_available())
    {
        switch(state){
            ...
            case CommsState_CRC:{
                ...
                uint8_t computed_crc = comms_compute_crc(&temporary_packet);
                ...
                if(comms_is_single_byte_packet(&temporary_packet, PACKET_RETX_DATA0) != 0){
                    comms_write(&last_transmitted_packet);
                    state = CommsState_Length;
                    break;
                }

                if(comms_is_single_byte_packet(&temporary_packet, PACKET_ACK_DATA0) != 0){
                    state = CommsState_Length;
                    break;
                }
